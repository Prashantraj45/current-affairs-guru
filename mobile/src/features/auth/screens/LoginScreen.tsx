...
export default function LoginScreen() {
  ...
  const handleDevBypass = () => {
    setAuth({ uid: 'dev-user', provider: 'google', email: 'dev@test.com', displayName: 'Dev User', photoUrl: '' }, 'dev-token', 'dev-refresh');
  };
  return (
    <View style={s.c}>
      ...
      {__DEV__ && (
        <Pressable style={[s.btn, s.dev]} onPress={handleDevBypass}>
          <Text style={s.devText}>Dev Bypass (skip login)</Text>
        </Pressable>
      )}
    </View>
  );
...
const s = StyleSheet.create({
  ...
  dev: { backgroundColor: '#2a2a4e', borderWidth: 1, borderColor: '#6366f1' }, devText: { color: '#6366f1', fontWeight: '600', fontSize: 14 },
});
